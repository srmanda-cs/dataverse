...
The Dataverse software has been tested with Solr version 9.8.1. Future releases in the 9.x series are likely to be compatible. Please get in touch (:ref:`support`) if you are having trouble with a newer version.
...
Become the ``solr`` user and then download and configure Solr::
        ...
        su - solr
        cd /usr/local/solr
        wget https://archive.apache.org/dist/solr/solr/9.8.1/solr-9.8.1.tgz
        tar xvzf solr-9.8.1.tgz
        cd solr-9.8.1
...
You should already have a "dvinstall.zip" file that you downloaded from https://github.com/IQSS/dataverse/releases . Unzip it into ``/tmp``. Then copy the files into place::
        ...
        cp /tmp/dvinstall/schema*.xml /usr/local/solr/solr-9.8.1/server/solr/collection1/conf
        cp /tmp/dvinstall/solrconfig.xml /usr/local/solr/solr-9.8.1/server/solr/collection1/conf
...
A Dataverse installation requires a change to the ``jetty.xml`` file that ships with Solr. Edit ``/usr/local/solr/solr-9.8.1/server/etc/jetty.xml`` , increasing ``requestHeaderSize`` from ``8192`` to ``102400``
...
Finally, you need to tell Solr to create the core "collection1" on startup::
        ...
        echo "name=collection1" > /usr/local/solr/solr-9.8.1/server/solr/collection1/core.properties
